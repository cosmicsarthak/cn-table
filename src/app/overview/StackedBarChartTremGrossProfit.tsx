...
// Define props for StackedBarChartTremGrossProfit to receive orders, loading, and error
interface StackedBarChartTremGrossProfitProps {
  orders: Order[]
  loading: boolean
  error: string | null
}

export const StackedBarChartTremGrossProfit: React.FC<StackedBarChartTremGrossProfitProps> = ({
                                                                          orders,
                                                                          loading,
                                                                          error,
                                                                        }) => {
  ...
  const { chartdata, categories } = useMemo(() => {
    ...
    filteredOrders.forEach((order) => {
      ...
      const grossProfit = order.grossProfit || 0;
      ...
      monthData[customer] = (monthData[customer] || 0) + grossProfit;
  ...
  // Custom tooltip for BarChart to show sum of grossProfit
  ...
  return (
    <div className="flex flex-col gap-16">
      ...
      <div className="flex flex-col gap-4">
        <p className="mx-auto font-mono text-sm font-medium">Stacked <span className={"bg-black font-bold p-2 uppercase rounded-xl border text-white"}>Gross Profit</span> for <span className={"font-bold"}>{selectedYear}</span></p>
        <BarChart
          type="stacked"
